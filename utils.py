...
def train_dev_test_split(data, label, train_frac, dev_frac):
    ...
    random_state=random.seed(sys.argv[1])
    x_train, x_dev_test, y_train, y_dev_test = train_test_split(
        data, label, test_size=dev_test_frac, shuffle=False, random_state=random_state
    )
    x_test, x_dev, y_test, y_dev = train_test_split(
        x_dev_test, y_dev_test, test_size=(dev_frac) / dev_test_frac, shuffle=False, random_state=random_state
    )
